        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
                  ...
                  <TableRow key={item.employee}>
                    ...
                    {item.shifts.map((shift, index) => (
                      <TableCell key={index} className="text-center">
                        <Badge variant="secondary" className={cn('w-8 h-8 flex items-center justify-center font-bold', getShiftClass(shift))}>
                          {shift}
                        </Badge>
                      </TableCell>
                    ))}
            ...
            <div className="mt-4 flex items-center gap-4 text-sm text-muted-foreground">
                ...
                <span className="flex items-center gap-2"><Badge className={cn('font-bold', getShiftClass('S1'))}>S1</Badge> Shift 1 (07:00-15:00)</span>
                <span className="flex items-center gap-2"><Badge className={cn('font-bold', getShiftClass('S2'))}>S2</Badge> Shift 2 (15:00-23:00)</span>
                <span className="flex items-center gap-2"><Badge className={cn('font-bold', getShiftClass('S3'))}>S3</Badge> Shift 3 (23:00-07:00)</span>
                <span className="flex items-center gap-2"><Badge className={cn('font-bold', getShiftClass('L'))}>L</Badge> Libur</span>
            </div>
          </CardContent>
        </Card>
            ...
            <Card>
                ...
                <CardContent>
                    <ChartContainer config={chartConfig} className="w-full h-[180px]">
                        <BarChart accessibilityLayer data={attendanceStats} layout="vertical" margin={{ left: 10 }}>
                            <YAxis
                                dataKey="type"
                                type="category"
                                tickLine={false}
             ...
             <Card>
                <CardHeader>
                    ...
                    <CardDescription>Permintaan tukar shift yang menunggu persetujuan.</CardDescription>
                </CardHeader>
                <CardContent>
